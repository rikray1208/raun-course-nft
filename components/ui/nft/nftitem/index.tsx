...
const NftItem: FC<NftItemProps> = ({nft}) => {
    const {image, name, attributes, description} = nft
            ...
            <div className="flex-1 bg-white p-6 flex flex-col justify-between">
                ...
                <div className="overflow-hidden mb-4">
                    <dl className="-mx-4 -mt-4 flex flex-wrap">
                        ...
                        {attributes.map(el =>
                            <div key={el.trait_type} className="flex flex-col px-4 pt-4">
                                <dt className="order-2 text-sm font-medium text-gray-500">{el.trait_type}</dt>
                                <dd className="order-1 text-xl font-extrabold text-indigo-600">{el.value}</dd>
                            </div>
                        )}
                    </dl>
                </div>
